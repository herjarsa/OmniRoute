...
├── executors/            # Per-provider HTTP executors (75+ files)
...
|-----------|-------|---------|
| `executors/` | 75 | One file per provider (openai.ts, anthropic.ts, etc.) |
| `handlers/` | ~15 | Request entry points (chatCore, responses, embeddings) |
| `services/` | 117 | Routing, caching, rate limiting, refresh, etc. |
...
Each provider has its own executor (e.g., `executors/openai.ts`, `executors/anthropic.ts`). The executor:
...
Subclasses only override what differs from the OpenAI-compatible default:

```ts
// open-sse/executors/openai.ts
export class OpenAIExecutor extends BaseExecutor {
  // Inherits everything — OpenAI is the default behavior
}

// open-sse/executors/anthropic.ts
export class AnthropicExecutor extends BaseExecutor {
  override buildHeaders(request): Headers {
    const headers = super.buildHeaders(request);
    headers.set("anthropic-version", "2023-06-01");
    headers.set("x-api-key", this.connection.apiKey);  // not Bearer
    return headers;
  }
}
```
...
**OpenAI-compatible** (most providers): no override needed.

```ts
// open-sse/executors/openai.ts
export class OpenAIExecutor extends BaseExecutor {
  // Inherits everything — OpenAI is the default
}

getExecutor("openai")  // Returns OpenAIExecutor
```

**Custom URL** (provider has different base URL):

```ts
// open-sse/executors/together.ts
export class TogetherExecutor extends BaseExecutor {
  override buildUrl(request): string {
    return "https://api.together.xyz/v1/chat/completions";
  }
}
```

**Custom auth** (e.g., not Bearer token):
...
**Custom request body** (e.g., Anthropic has separate system field):

```ts
override transformRequest(body): Record<string, unknown> {
  const { system, messages, ...rest } = body;
  return {
    ...rest,
    messages,
    system: system ?? "",
...
```ts
// open-sse/mcp-server/tools/providerTools.ts
export const providerTools = [
  {
    ...
    handler: async (args, ctx) => {
      return await listProviders(args.filter);
    },
  },
  // ...
];
